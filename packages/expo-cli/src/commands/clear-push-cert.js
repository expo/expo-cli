...
export default program => {
  program
    .command('clear-push-cert [project-dir]')
    .option('-r --revoke', 'Revoke selected push certificate on developer.apple.com')
    .description(`Remove Push Certificate from Expo's servers`)
    .asyncActionProjectDir(async (projectDir, options) => {
      ...
      if (options.revoke) {
        const revokeQuestion = [
          {
            type: 'confirm',
            name: 'confirm',
            message: `Please confirm you wish to remove the push certificate from Expo's servers and permanently revoke the one you select.`,
          },
        ];
        ...
        if (confirm) {
          ...
          log.warn('The selected push certificate has successfully been revoked.');
        }
      }
      if (!options.revoke) {
        const clearQuestion = [
          {
            type: 'confirm',
            name: 'confirm',
            message: `Please confirm you wish to remove the push certificate from Expo's servers.`,
          },
        ];
