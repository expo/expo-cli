...
export default program => {
  program
    .command('clear-push-key [project-dir]')
    .option('-r --revoke', 'Revoke selected push notification key on developer.apple.com')
    .description(`Remove Push Notification Key from Expo's servers`)
    .asyncActionProjectDir(async (projectDir, options) => {
      ...
      if (options.revoke) {
        const revokeQuestion = [
          {
            type: 'confirm',
            name: 'confirm',
            message: `Please confirm you wish to remove the push notification key from Expo's servers and permanently revoke the one you select.`,
          },
        ];
        ...
        if (confirm) {
          ...
          log.warn('The selected push notification key has successfully been revoked.');
        }
      }
      if (!options.revoke) {
        const clearQuestion = [
          {
            type: 'confirm',
            name: 'confirm',
            message: `Please confirm you wish to remove the push notification key from Expo's servers.`,
          },
        ];
