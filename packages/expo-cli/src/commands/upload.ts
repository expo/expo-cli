...
import { downloadEASFile, downloadFile } from './upload/utils';
...
export default function(program: Command) {
  program
    ...
    .asyncActionProjectDir(async (projectDir: string, options: AndroidSubmitCommandOptions) => {
      await uploadAndroidAsync(projectDir, { options });
    });

  function addCommonIosUploadOptions(command: any) {
    return (
      command
  ...
  addCommonIosUploadOptions(
    program
      ...
      .asyncActionProjectDir(async (projectDir: string, options: IosPlatformOptions) => {
        await uploadIosAsync(projectDir, { options, useEAS: false });
      })
  );
  ...
  addCommonIosUploadOptions(
    program
      .command('eas:upload [projectDir]')
      // .alias('eui')
      // .option(
      //   '-p --platform <platform>',
      //   'Uploads builds for specified platform: ios, android, all',
      //   /^(all|android|ios)$/i
      // )
      .asyncActionProjectDir(async (projectDir: string, options: IosPlatformOptions) => {
        await uploadIosAsync(projectDir, { options, useEAS: true });
      })
  );
}

async function uploadIosAsync(
  projectDir: string,
  { options, useEAS }: { options: IosPlatformOptions; useEAS: boolean }
) {
  try {
...
async function uploadAndroidAsync(
  projectDir: string,
  { options }: { options: AndroidSubmitCommandOptions }
) {
  // TODO: remove this once we verify `fastlane supply` works on linux / windows
