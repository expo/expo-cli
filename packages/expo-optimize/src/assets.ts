...
async function readAssetJsonAsync(
  ...
  if (!fileExists(assetJson.file)) {
    console.log(
      chalk.magenta`\u203A Creating ${chalk.italic(
...
async function optimizeImageAsync(inputPath: string, quality: number): Promise<string> {
  console.log(chalk.bold`\u203A Optimizing ${inputPath}`);
...
export async function optimizeAsync(
  ...
  console.log(chalk.magenta`\u203A Optimizing assets...`);
  ...
  if (totalSaved === 0) {
    console.log(chalk.yellow`\u203A No assets optimized. Everything is fully compressed!`);
  } else {
    console.log(
      chalk.bold`\u203A Finished compressing assets. ${chalk.green(prettyBytes(totalSaved))} saved.`
    );
  }
