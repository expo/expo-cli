...
export type Command<IsDetached extends boolean = false> = {
  ...
  options?: Array<
    CommandOption<IsDetached extends true ? () => OptionValue : (ctx: Config) => OptionValue>
  >;
};
...
export type UserConfig = Pick<
  Config,
  'reactNativePath' | 'project' | 'assets' | 'dependencies' | 'platforms' | 'commands'
> & {
...
export type UserDependencyConfig = {
  ...
  dependency: Pick<Dependency, 'platforms' | 'assets' | 'hooks' | 'params'>;
