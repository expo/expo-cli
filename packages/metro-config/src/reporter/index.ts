...
// @ts-ignore
import { AmbiguousModuleResolutionError, Terminal } from 'metro-core';
import path from 'path';

import * as reporting from './reporting';

type BundleProgress = {
  bundleDetails: reporting.BundleDetails;
...
class TerminalReporter {
  ...
  constructor(terminal: Terminal) {
    ...
    this._scheduleUpdateBundleProgress = throttle((data: any) => {
  ...
  _getBundleStatusMessage(
    ...
    const bundleTypeColor =
      phase === 'done' ? chalk.green : phase === 'failed' ? chalk.red : chalk.yellow;
    const progress =
      phase === 'in_progress'
  ...
  _logCacheDisabled(reason: reporting.GlobalCacheDisabledReason): void {
