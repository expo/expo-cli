...
module.exports = function webpackConfig(projectRoot) {
  const pckg = require(path.resolve(projectRoot, 'package.json'));
  const locations = {
    // Shouldn't change
    root: projectRoot,
    output: path.resolve(projectRoot, 'web'),
    rootHtml: path.resolve(projectRoot, 'web/index.html'),
    packageJson: path.resolve(projectRoot, 'package.json'),
    appMain: path.resolve(projectRoot, pckg.main),
  };
  ...
  const includeModule = module => {
    return path.resolve(workspaceRoot || locations.root, 'node_modules', module);
  };

  const babelLoaderConfiguration = {
    test: /\.jsx?$/,
    include: workspaceRoot
      ? [locations.root, path.resolve(workspaceRoot, 'node_modules')]
      : [locations.root],
    use: {
  ...
  const htmlLoaderConfiguration = {
    ...
    include: [path.resolve(projectRoot, './assets')],
  };
  ...
  function getWebModule(moduleName, initialRoot, logTag) {
    return function(res) {
      ...
      console.log('includeModule(', initialRoot, moduleName, ')', res);
