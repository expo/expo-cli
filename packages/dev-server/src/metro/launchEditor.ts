...
function getArgumentsForLineNumber(
  ...
  switch (path.basename(editor)) {
    ...
    case 'charm':
    case 'idea':
      return [fileName, lineNumber, coulmnNumber].filter(Boolean).join(':');
    case 'joe':
    case 'emacs':
    case 'emacsclient':
    ...
    case 'code':
      return addWorkspaceToArgumentsIfExists(
        ['-g', [fileName, lineNumber, coulmnNumber].filter(Boolean).join(':')],
        workspace
      );
...
function launchEditor(
  ...
  if (lineNumber) {
    args = args.concat(getArgumentsForLineNumber(editor, fileName, lineNumber, column, workspace));
  } else {
    args.push(fileName);
  }
  ...
  if (process.platform === 'win32' && !WINDOWS_FILE_NAME_WHITELIST.test(fileName.trim())) {
    console.error(`Could not open ${path.basename(fileName)} in the editor.`);
    console.info(
  ...
  console.info(`Opening ${chalk.underline(fileName)} with ${chalk.bold(editor)}`);
  ...
  _childProcess.on('exit', errorCode => {
    ...
    if (errorCode) {
      console.error('Your editor exited with an error!');
