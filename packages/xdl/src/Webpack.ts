...
async function startNextJsAsync({
  ...
  port: number;
  dev: boolean;
  expoConfig: Web.WebpackConfiguration;
}): Promise<DevServer> {
  let next;
  try {
    const { exp } = await ProjectUtils.readConfigJsonAsync(projectRoot);
...
function _createNextJsConf({
  ...
  return {
    ...
    pageExtensions: expoConfig.resolve!.extensions!.map((string: string) => string.substr(1)),
    ...userNextConfigJs,
    ...
    webpack: (nextjsConfig: Web.WebpackConfiguration, options: any) => {
      let newConfig = {
        ...nextjsConfig,
        module: {
          ...nextjsConfig.module,
